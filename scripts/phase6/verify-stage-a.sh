...
# Read-only Stage A runtime acceptance. Runtime credentials and endpoints are
...
for index in "${!namespaces[@]}"; do
  ...
  kube_json "$quota_json" -n "$namespace" get resourcequota stage-a-budget
...
assert quota.get("metadata", {}).get("name") == "stage-a-budget"
...
kube_json "$cluster_policies_json" get clusterpolicy \
  phase6-workload-baseline sealed-secret-strict-scope
...
for report in reports:
    ...
    if namespace in required and any(r.get("policy") == "phase6-workload-baseline" for r in results):
        assert total > 0
...
for index in "${!namespaces[@]}"; do
  ...
  kube_json "$deployment_json" -n "$namespace" get deployment stage-a-smoke
...
assert metadata.get("name") == "stage-a-smoke"
...
pattern = rf"{re.escape(registry_host)}/platform-demo/stage-a-smoke@(sha256:[0-9a-f]{{64}})"
...
selected = [item for item in ingresses if item.get("metadata", {}).get("labels", {}).get("app.kubernetes.io/name") == "stage-a-smoke"]
assert len(selected) == 1
...
assert payload.get("service") == "stage-a-smoke"
...
for index in "${!namespaces[@]}"; do
  ...
  api_get "$rancher_endpoint" "$PHASE6_RANCHER_REVIEWER_TOKEN_FILE" \
    "/k8s/clusters/local/apis/apps/v1/namespaces/${namespaces[index]}/deployments/stage-a-smoke" \
    "$rancher_workload"
  if ! "$python_bin" -c '
import json, sys
item = json.load(open(sys.argv[1], encoding="utf-8"))
expected = int(sys.argv[2])
assert item.get("metadata", {}).get("name") == "stage-a-smoke"
...
api_get "$harbor_endpoint" "$PHASE6_HARBOR_REVIEWER_TOKEN_FILE" \
  '/api/v2.0/projects/platform-demo/repositories/stage-a-smoke/artifacts?page=1&page_size=25&with_scan_overview=true' \
  "$harbor_artifacts"
if ! "$python_bin" -c '
...
api_get "$harbor_endpoint" "$PHASE6_HARBOR_REVIEWER_TOKEN_FILE" \
  "/api/v2.0/projects/platform-demo/repositories/stage-a-smoke/artifacts/${artifact_digest}/additions/vulnerabilities" \
...
metric_query='sum by (environment) (stage_a_smoke_build_info)'
...
loki_query='sum(count_over_time({cluster="management", namespace="demo-dev", application="stage-a-smoke"} |= "\"marker\":\"stage_a_smoke\"" [15m]))'
encoded_loki_query=$(
  "$python_bin" -c 'import sys, urllib.parse; print(urllib.parse.quote(sys.argv[1], safe=""))' \
...
printf '[PASS] Phase 6 Stage A runtime acceptance completed.\n'

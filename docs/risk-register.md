...
| R-016 | Object-storage bucket and credential are outside Terraform ownership | Medium/Observed | High | Manual resource drifts, expires, or survives teardown | Documented provider-gap exception, external credential boundary, live local/off-cluster snapshot proof, explicit inventory | Reconcile manually, rotate/recreate the scoped credential, and delete retained objects through the approved console path | Recovery owner |
| R-017 | Operator address changes after exact `/32` SSH restriction | Observed | High | All three strict SSH/TCP probes are denied from the current source | Runtime-derived exact allowlist, timed rollback, authenticated source evidence, external host-key pins; authorized recovery tested | Repeat the rollback-protected allowlist workflow; use VPN/bastion for durable production access | Security owner |

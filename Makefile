...
.PHONY: help bootstrap-tools install-hooks validate validate-negative pre-commit secret-scan ci discover \
	...
	infra-init infra-plan infra-apply inventory configure verify-hosts verify-cluster \
...
infra-init infra-plan infra-apply inventory configure verify-hosts verify-cluster bootstrap-gitops platform-status stage-a-verify register-clusters stage-b-verify app-test app-build supply-chain-verify promote verify fault backup restore-test cost-report collect-evidence sanitize-evidence destroy:
	@$(PWSH) -NoLogo -NoProfile -NonInteractive -File scripts/quality/phase-gate.ps1 -Target "$@" -Arguments "CLUSTER=$(CLUSTER) FROM=$(FROM) TO=$(TO) DIGEST=$(DIGEST) TEST=$(TEST) TARGET=$(TARGET)"

#!/usr/bin/env python3
"""Behavioral and evidence-safety contract for Platform runtime acceptance."""
...
SCRIPT = ROOT / "scripts" / "phase6" / "verify-platform-demo.sh"
EXPECTED_APPS = (
    "platform-root",
    "platform-project",
...
class Phase6StageARuntimeVerifierTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.temporary = tempfile.TemporaryDirectory(
            prefix="phase6-platform-", dir=runtime_root
        )
    ...
    def _write_fake_clients(self) -> None:
        write_executable(
            ...
            if args[:2] == ["get", "resourcequota"]:
                ...
                emit({"metadata":{"name":"platform-budget"},"spec":{"hard":hard}}); raise SystemExit
            ...
            if args[:2] == ["get", "clusterpolicy"]:
                items = []
                for name in ("workload-baseline", "sealed-secret-strict-scope"):
            ...
            if args[:2] == ["get", "policyreports.wgpolicyk8s.io"]:
                emit({"items":[{"metadata":{"namespace":ns},"summary":{"pass":1,"fail":0,"warn":0,"error":0,"skip":0},"results":[{"policy":"workload-baseline","result":"pass"}]} for ns in ("demo-dev","demo-staging","demo-prod")]}); raise SystemExit
        ...
        write_executable(
            ...
            if host.startswith("demo-"):
                ...
                write({"service":"platform-demo","environment":environment,"version":"fixture"}); raise SystemExit
            if host.startswith("rancher."):
                ...
                if path.startswith("/k8s/clusters/local/apis/apps/v1/namespaces/"):
                    ...
                    write({"metadata":{"name":"platform-demo"},"spec":{"replicas":replicas},"status":{"readyReplicas":replicas}}); raise SystemExit
    ...
    def test_success_is_fixed_sanitized_and_read_only(self) -> None:
        ...
        expected_lines = (
            ...
            "[PASS] Phase 6 Platform runtime acceptance completed.",
        )

#!/usr/bin/env python3
"""Offline contract tests for the separate Phase 6 Stage A smoke workload."""
...
APP = ROOT / "applications" / "stage-a-smoke"
CHART = APP / "chart"
ENVIRONMENTS = {
    "dev": ("demo-dev", 1),
...
class Phase6StageASmokeContractTests(unittest.TestCase):
    def test_source_is_small_static_deterministic_and_passes_go_tests(self) -> None:
        ...
        for contract in (
            ...
            "stage_a_smoke_requests_total",
            "stage_a_smoke_build_info",
    ...
    def test_dockerfile_is_multistage_scratch_nonroot_and_fail_closed(self) -> None:
        ...
        builder = versions["images"]["stage_a_smoke_builder_reference"]
    ...
    def test_certificate_bootstrap_is_separate_from_workload_activation(self) -> None:
        ...
        self.assertEqual(
            ...
            ["stage-a-dev.95-133-252-214.sslip.io"],
        )
        self.assertEqual(certificate["spec"]["secretName"], "stage-a-smoke-staging-tls")
    ...
    def test_admitted_render_has_exact_workload_tls_metrics_and_security_contract(
        ...
        for environment, (namespace, replicas) in ENVIRONMENTS.items():
            with self.subTest(environment=environment):
                ...
                self.assertRegex(
                    container["image"],
                    r"^harbor\.[0-9-]+\.sslip\.io/platform-demo/stage-a-smoke@sha256:[0-9a-f]{64}$",
                )
                ...
                self.assertEqual(
                    ingress["spec"]["tls"][0]["secretName"], "stage-a-smoke-tls"
                )
                monitor = next(
    ...
    def test_network_policies_are_only_exact_ingress_exceptions(self) -> None:
        ...
        self.assertEqual(
            ...
            {"stage-a-smoke-traefik-ingress", "stage-a-smoke-prometheus-ingress"},
        )
    ...
    def test_values_schema_is_strict_and_host_environment_is_bound(self) -> None:
        ...
        invalid = helm_template(
            "dev", *admitted_settings(), "hostname=stage-a-prod.192-0-2-10.sslip.io"
        )

...
| Object-storage application compatibility, lifecycle policy, request/egress pricing, and later-service credentials are unverified | FUTURE BLOCKER — Phase 5 | Loki/Velero/Longhorn configuration could fail or exceed allowance | Run non-destructive compatibility and pricing checks with separately scoped credentials before later use |

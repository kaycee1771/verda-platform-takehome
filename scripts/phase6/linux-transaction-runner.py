...
class LinuxRunnerBoundary:
 def __init__(self,*,platform:str=sys.platform,uid:int|None=None,config_home:pathlib.Path|None=None,
              state_home:pathlib.Path|None=None,stat_probe:Callable[[pathlib.Path],Any]=os.lstat,
              allow_test_probe:bool=False)->None:
  if platform!="linux":refuse("Linux transaction runner is unavailable on this platform")
  if stat_probe is not os.lstat and allow_test_probe is not True:refuse("custom filesystem probe is test-only")
  self.uid=PRODUCTION_UID if uid is None else uid
  if self.uid!=PRODUCTION_UID and allow_test_probe is not True:refuse("production runner UID differs")
  ...
  self.allow_test_probe=allow_test_probe;self._root_fds:list[int]=[]
 def validate_roots(self)->None:
  if self.config_root==self.state_root:refuse("runner config and state roots are not distinct")
  for path in (self.config_root,self.state_root):
   ...
   for ancestor in reversed(path.parents):
    info=self.stat_probe(ancestor)
    if (not stat.S_ISDIR(info.st_mode) or info.st_uid not in {0,self.uid}
        or stat.S_IMODE(info.st_mode)&0o022):refuse("runner ancestor owner/type/writability differs")
   ...
   if not self.allow_test_probe:
    fd=os.open(path,os.O_RDONLY|os.O_DIRECTORY|os.O_NOFOLLOW)
    held=os.fstat(fd)
    if (held.st_dev,held.st_ino)!=(value.st_dev,value.st_ino):os.close(fd);refuse("runner root identity changed during no-follow open")
    self._root_fds.append(fd)
 def validate_manifest(self,manifest:dict[str,Any],files:dict[str,bytes],metadata:dict[str,Any])->str:
  ...
  for path,raw in files.items():
   ...
   info=metadata[path]
   if (not stat.S_ISREG(info.st_mode) or stat.S_IMODE(info.st_mode) not in {0o444,0o644}
       or info.st_nlink!=1 or info.st_uid!=self.uid):refuse("runner archive file metadata differs")
 ...
 def validate_reviewed_commit(self,manifest:dict[str,Any],repository_probe:Callable[[],tuple[str,bool]])->None:
  head,clean=repository_probe()
  if type(head) is not str or not COMMIT.fullmatch(head) or clean is not True or manifest.get("reviewed_commit")!=head:
